...
describe('клавіатурний газ', () => {
  it('W піднімає газ, S опускає', () => {
    expect(stepKeyboardThrottle(0.4, 1, 0, 0.1, 1)).toBeGreaterThan(0.4)
    expect(stepKeyboardThrottle(0.4, 0, 1, 0.1, 1)).toBeLessThan(0.4)
  })

  it('трещітка: відпустив — газ стоїть РІВНО там, де поставив', () => {
    let t = 0.63
    for (let i = 0; i < 240; i++) t = stepKeyboardThrottle(t, 0, 0, 1 / 60, 0.85)
    expect(t).toBe(0.63)
  })

  it('на землі газ не наростає сам — зліт робить пілот', () => {
    let t = 0
    for (let i = 0; i < 120; i++) t = stepKeyboardThrottle(t, 0, 0, 1 / 60, 0.85)
    expect(t).toBe(0)
  })

  it('газ завжди в межах [0,1]', () => {
    expect(stepKeyboardThrottle(0.98, 1, 0, 1, 5)).toBeLessThanOrEqual(1)
    expect(stepKeyboardThrottle(0.02, 0, 1, 1, 5)).toBeGreaterThanOrEqual(0)
  })
})
 ...
 * Відтворюємо повний ланцюжок гри — латчений клавіатурний газ + ANGLE-нахил —
 * і перевіряємо, що ніс униз дає саме політ уперед, а не набір висоти.
 * Газ фіксований на зависанні: у нахилі борт помірно осідає — це компенсує
 * пілот, а не асист (трещітка з 07.08.2026).
 */
function flyForward(seconds: number, tiltStick: number, maxTilt = 45) {
  ...
  for (let i = 0; i < seconds / PHYSICS_DT; i++) {
    throttle = stepKeyboardThrottle(throttle, 0, 0, PHYSICS_DT, 0.85)
...
describe('політ уперед', () => {
  it('ніс униз дає рух уперед, а не вгору', () => {
    ...
    // на трещітці газу борт у нахилі осідає (це компенсує пілот),
    // але претензія була «улітає ВГОРУ» — вгору тікати не має
    expect(r.climb).toBeLessThan(5)
    expect(Math.abs(r.climb)).toBeLessThan(r.forward * 0.5)
  })
  ...
  it('на фіксованому газі в нахилі зниження є, але контрольоване', () => {
    const r = flyForward(10, -1)
    expect(Math.abs(r.verticalSpeed)).toBeLessThan(8)
  })
